...
`DISCORD_BOT_TOKEN`이 설정돼 있으면 백엔드 서버가 뜰 때 봇도 같이 디스코드에 접속해서 서버 멤버 목록에 **온라인**으로 표시됩니다 (별도 프로세스/컨테이너 없이 백엔드 안에서 백그라운드로 접속 유지). 토큰이 비어있거나 잘못됐어도 서버 자체는 정상 기동하고, 이벤트 등록 REST 기능도 그대로 동작합니다 — 봇 접속은 순전히 부가 기능(온라인 표시)이라 실패해도 안전합니다.
