...
- [x] 참가자 체크리스트 API — `GET /api/users/checklist`(관리자 전용)가 승인된 참가자별로 발표 횟수(승인된 세션 수), 최근 발표 자료 작성 완료 여부(concept/example/demo/summary 4개 필드 전부 채워졌는지), 디스코드 참여 여부를 한 번에 반환. `applications.discord_id`(선택, 마이그레이션 `37067d11f048`) 추가 — `/apply`에서 디스코드 사용자 ID를 선택 입력받고, `discord_events.py`의 `check_guild_membership()`이 봇으로 서버 멤버 목록을 조회해 자동 판정(아이디 미입력/봇 미설정/조회 실패 시엔 "확인 불가"로 구분 표시, 절대 신청 자체를 막지 않음). 디스코드 확인은 `asyncio.gather`로 병렬 처리. Docker로 401(가짜 토큰)까지 실제 도달 확인 + 관리자 아닌 사용자 403 확인
