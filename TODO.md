...
- [x] `study_settings` 테이블(싱글톤) + `GET/PATCH /api/settings` — 참가 신청 기간(시작/마감)과 설명회 참여 가능 시간 옵션 목록을 관리자가 `/admin`에서 직접 수정 (기존 `.env`의 `APPLICATION_OPENS_AT`/`CLOSES_AT`는 최초 1회 시드 값으로만 쓰이고, 이후엔 DB 값이 우선함)
