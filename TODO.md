...
- [x] 참가자 체크리스트 API — `GET /api/users/checklist`(관리자 전용)가 승인된 참가자별로 발표 횟수(승인된 세션 수), 최근 발표 자료 작성 완료 여부(concept/example/demo/summary 4개 필드 전부 채워졌는지), 디스코드 참여 여부를 한 번에 반환. `applications.discord_id`(선택, 마이그레이션 `37067d11f048`)는 참고용으로만 수집
- [x] 디스코드 참여 여부는 자동 조회 대신 관리자가 직접 표시 — 처음엔 봇으로 서버 멤버 목록을 자동 조회했는데, 관리자가 직접 체크하고 싶다고 해서 변경. `applications.discord_joined`(불리언, 기본 false, 마이그레이션 `94de5a67d429`, 기존 행 있는 테이블에도 `server_default`로 안전하게 적용됨) 추가, `PATCH /api/users/checklist/{user_id}`(관리자 전용)로 토글. `discord_events.py`의 `check_guild_membership()`은 더 안 쓰여서 제거. `/admin` 체크리스트에서 뱃지 클릭으로 참여/미참여 토글. Docker로 토글 반영·재조회 확인 + 관리자 아닌 사용자 403 확인
- [x] 발표 신청 시 날짜별 슬롯 자동 리필 — 한 명이 신청하면 그 날짜의 빈 슬롯이 없어져서 바로 "잠기는" 문제가 있었음. `claim_session`에서 신청 처리 후 `_ensure_open_slot()`이 그 날짜의 정원(`MAX_PRESENTERS_PER_DATE`, 3명)이 아직 안 찼는데 빈 슬롯이 없으면 자동으로 하나 더 열어줌(디스코드 웹훅으로도 안내). 정원이 다 차면 더 이상 안 열림. Docker로 1번째 신청→자동 리필→2번째→리필→3번째→더 이상 안 열림(정원 도달)까지 curl로 확인
- [x] 발표 피드백 입력 UI — 백엔드 API(`POST/GET /api/sessions/{id}/feedbacks`)는 있었지만 작성할 수 있는 화면이 없었음. 세션 상세의 탭에 "피드백" 탭 추가(Q&A 탭과 동일한 패턴 — 목록+작성 폼, 비로그인 시 로그인 안내). Docker로 피드백 등록·조회 curl로 확인
- [x] 발표자 본인 자료 작성 페이지 — 지금까지 발표 자료(개념/예시/실습/정리/퀴즈/자료링크)는 관리자만 수정 가능했음. `PATCH /api/sessions/{id}` 권한을 관리자 전용에서 "관리자 전체 허용 + 발표자 본인은 자료 항목만 허용"으로 완화(`PRESENTER_EDITABLE_FIELDS`, 날짜/주제/상태는 여전히 관리자 전용 — 위반 시 403). 새 페이지 `/sessions/:id/edit`(`SessionEdit.tsx`)에서 발표자가 직접 작성, 세션 상세에 본인/관리자에게만 보이는 "발표 자료 작성/수정" 링크 추가. Docker로 발표자 본인 성공/주제 변경 시도 403/다른 학생 403/관리자는 전부 허용 4가지 curl로 확인
...
- [x] `/apply` — 로그인 필요, 학번/이름/전화번호/디스코드 사용자 ID(선택)/분야(2개+)/설명회 가능 시간(관리자 설정 옵션 라디오)/개인정보 동의/규칙 동의, 신청 기간 아니면 안내 문구
- [x] `/admin` — 설정(신청 기간+설명회 옵션+디스코드), 신청 승인/거절+SMS(카드형 UI), 참가자 role 변경, **참가자 체크리스트**(발표 횟수/자료 작성/디스코드 참여), **날짜만 입력하는 발표 슬롯 생성**(같은 날짜 3개까지), 세션 수정(**발표 주제 포함**)/삭제 폼, **설명회/공지/회의 일정 이벤트 CRUD**, **벌금 부과/면제 폼**, **출석 체크 UI** — 전부 관리자 전용 페이지 안에만 존재. 삭제는 확인창(`window.confirm`) 후 실행, 관련 질문/피드백/출석/벌금이 있으면 백엔드가 400으로 막고 "상태를 취소로 바꾸라"고 안내(외래키 제약 위반이 500으로 새는 것 방지), 등록된 디스코드 이벤트도 삭제 시 같이 정리
- [x] 벌금 사유에 "숙제안함" 추가 — `FineReason` enum에 `homework_incomplete`("숙제안함") 신설(마이그레이션 `44d5f842a78a`, MySQL enum이라 autogenerate가 못 잡아서 수동 작성). Docker로 다운그레이드→업그레이드 왕복 확인, 새 사유로 벌금 생성까지 curl로 확인
...
- [x] 로딩/에러 상태 UI — `Spinner.tsx`(공통 스피너), `ErrorMessage.tsx`(메시지+다시 시도 버튼) 컴포넌트 신설. 적용: `AuthGuard`(로그인 확인 중 빈 화면 대신 스피너), `SessionDetail`/`MemberDetail`(텍스트 대신 스피너+실패 시 에러+재시도, 잘못된 id로 무한 로딩되던 문제도 같이 해결), `Members`/`Fines`/`Schedule`(로딩 중 빈 목록이 "없음"으로 오인되던 문제 해결). 브라우저에서 백엔드 끈 상태로 에러 메시지 확인 → 백엔드 켜고 "다시 시도" 클릭해 정상 복구 확인, 존재하지 않는 참가자 id 접근 시 에러 메시지 뜨는 것도 확인
